...
bool S2Polygon::DecodeUncompressed(Decoder* const decoder, bool within_scope) {
  if (decoder->avail() < 2 * sizeof(std::uint8_t) + sizeof(std::uint32_t)) return false;
  ClearLoops();
  ...
  const std::uint32_t num_loops = decoder->get32();
...
std::uint8_t GetCellEdgeIncidenceMask(const S2Cell& cell, const S2Point& p,
                               double tolerance_uv) {
  std::uint8_t mask = 0;
  R2Point uv;
...
vector<unique_ptr<S2Polyline>> S2Polygon::SimplifyEdgesInCell(
  ...
  for (int i = 0; i < a.num_loops(); ++i) {
    ...
    std::uint8_t mask0 = GetCellEdgeIncidenceMask(cell, *v0, tolerance_uv);
    ...
    for (int j = 1; j <= a_loop.num_vertices(); ++j) {
      ...
      std::uint8_t mask1 = GetCellEdgeIncidenceMask(cell, *v1, tolerance_uv);
      if ((mask0 & mask1) != 0) {
...
void S2Polygon::InitToCellUnionBorder(const S2CellUnion& cells) {
  ...
  if (num_loops() == 0) {
    ...
    S2_DCHECK_EQ(std::uint64_t{6} << (2 * S2CellId::kMaxLevel),
...
bool S2Polygon::DecodeCompressed(Decoder* decoder) {
  if (decoder->avail() < sizeof(std::uint8_t)) return false;
  ClearLoops();
